...
#define CHK(pk,fp,fkp) do{int*_f=(int*)(fp);uint64_t*_fk=(uint64_t*)(fkp);if(*_f)break;\
uint64_t_k[4];for(int_i=0;_i<4;_i++)_k[_i]=((uint64_t)(pk)[_i*8]<<56)|((uint64_t)(pk)[_i*8+1]<<48)|((uint64_t)(pk)[_i*8+2]<<40)|((uint64_t)(pk)[_i*8+3]<<32)|((uint64_t)(pk)[_i*8+4]<<24)|((uint64_t)(pk)[_i*8+5]<<16)|((uint64_t)(pk)[_i*8+6]<<8)|(pk)[_i*8+7];\
uint8_t_h[20];if(!d_pk2h160(_k,_h))break;\
for(int_t=0;_t<8;_t++){int_m=1;for(int_i=0;_i<20;_i++)if(_h[_i]!=d_targets[_t*20+_i]){_m=0;break;}\
if(_m){atomicExch(_f,1);for(int_i=0;_i<4;_i++)_fk[_i]=_k[_i];return;}}}while(0)

/* H21 */
__global__ void k21(void*f,void*fk){if(threadIdx.x||blockIdx.x)return;
    uint8_t p[32];d_sha256((const uint8_t*)"",0,p);CHK(p,f,fk);}
...
__global__ void k11(void*f,void*fk){if(threadIdx.x||blockIdx.x)return;
    ...
    d_sha256((const uint8_t*)(d_dict+pos),sl,p);CHK(p,f,fk);pos+=sl+1;}}
...
__global__ void k41(void*f,void*fk){if(threadIdx.x||blockIdx.x)return;
    ...
    d_sha256((const uint8_t*)s,sl,p);CHK(p,f,fk);
    char b[128];b[0]=(s[0]>='a'&&s[0]<='z')?(s[0]-32):s[0];
    for(int j=1;j<sl;j++)b[j]=s[j];d_sha256((const uint8_t*)b,sl,p);CHK(p,f,fk);pos+=sl+1;}}
...
__global__ void k28(uint64_t st,uint64_t cn,void*f,void*fk){
    ...
    uint8_t p[32];d_sha256(m,4,p);CHK(p,f,fk);}
...
__global__ void k3(uint64_t cn,void*f,void*fk){
    ...
    uint8_t p[32];d_sha256(m,10,p);CHK(p,f,fk);}
...
__global__ void k1(void*f,void*fk){
    ...
    d_sha256((const uint8_t*)ph,pl,p);CHK(p,f,fk);
    b[pl]='1';b[pl+1]='2';b[pl+2]='3';b[pl+3]=0;d_sha256((const uint8_t*)b,pl+3,p);CHK(p,f,fk);
    b[pl]='1';b[pl+1]=0;d_sha256((const uint8_t*)b,pl+1,p);CHK(p,f,fk);
    b[pl]='!';b[pl+1]=0;d_sha256((const uint8_t*)b,pl+1,p);CHK(p,f,fk);
    b[pl]='@';b[pl+1]=0;d_sha256((const uint8_t*)b,pl+1,p);CHK(p,f,fk);
    b[pl]='?';b[pl+1]=0;d_sha256((const uint8_t*)b,pl+1,p);CHK(p,f,fk);
    b[pl]='.';b[pl+1]=0;d_sha256((const uint8_t*)b,pl+1,p);CHK(p,f,fk);}
...
__global__ void k9(void*f,void*fk){
    ...
    for(int yr=2009;yr<=2013;yr++){if(*(int*)f)return;
        ...
        d_sha256((const uint8_t*)b,pl+yl,p);CHK(p,f,fk);}}
...
__global__ void k18(void*f,void*fk){
    ...
    uint8_t p[32];d_sha256((const uint8_t*)buf,al+1+bl,p);CHK(p,f,fk);}
